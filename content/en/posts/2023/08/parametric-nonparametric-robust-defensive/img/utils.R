...
ggsave_ <- function(name, plot = last_plot(), basic_theme = theme_bw(), multithemed = T, ext = "png",
  ...
  get_plot <- function(dark_mode) if (is.function(plot)) plot(dark_mode = dark_mode) else plot

  width <- width_px / dpi
  height <- height_px / dpi
  if (multithemed) {
    old_theme <- theme_set(basic_theme)
    ggsave(paste0(name, "-light.", ext), get_plot(dark_mode = FALSE), width = width, height = height, dpi = dpi)
    message("SAVED  : ./", paste0(name, "-light.", ext))
    theme_set(dark_mode(basic_theme, verbose = FALSE))
    p <- get_plot(dark_mode = TRUE)
    show(p)
    ggsave(paste0(name, "-dark.", ext), p, width = width, height = height, dpi = dpi)
...
regenerate_figures <- function() {
  ...
  for (file in list.files()) {
    if (endsWith(file, ".png") || endsWith(file, ".svg")) {
      file.remove(file)
    }
  }
