    ...
    class Program
    {
            ...
            int[] sampleSizes, ReferenceDistribution[] referenceDistributions)
        {
            ...
            var result = simulator.Run(estimators, probabilities, sampleSizes, referenceDistributions, progress =>
            {
                ...
                double etaMin = etaSec / 60.0;
                Logger.WriteLine($"Progress: {progress:N2}% Elapsed:{elapsedSec:N1}s ETA:{etaSec:N1}s ({etaMin:N1}m)");
            });
            sw.Stop();
            Logger.WriteLine($"Elapsed: {sw.Elapsed.TotalSeconds:N1}s ({sw.Elapsed.TotalMinutes:N1} min)");
