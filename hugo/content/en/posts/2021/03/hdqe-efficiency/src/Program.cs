...
namespace HarrellDavisEfficiency
{
    class Program
    {
        ...
        private void HarrellDavis()
        {
            var estimators = new IQuantileEstimator[] {HarrellDavisQuantileEstimator.Instance};

            var probabilities = Enumerable.Range(1, 99).Select(p => (Probability) (p / 100.0)).ToArray();
            int[] sampleSizes = Enumerable.Range(3, 58).ToArray();

            var sw = Stopwatch.StartNew();
            Run("Symmetric", estimators, probabilities, sampleSizes,
                SymmetricDistributionSet.Instance.Distributions);
            Run("RightSkewedUnimodal", estimators, probabilities, sampleSizes,
                RightSkewedUnimodalDistributionSet.Instance.Distributions);
            Run("RightSkewedBimodal", estimators, probabilities, sampleSizes,
                RightSkewedBimodalDistributionSet.Instance.Distributions);
            Run("SyntheticLatencyBrendanGregg", estimators, probabilities, sampleSizes,
                SyntheticLatencyBrendanGreggSet.Instance.Distributions);
            sw.Stop();
