...
draw.ratio <- function(index, comment, f) {
  draw(paste0("ratio", index), paste0("Ratio function between ", comment), f, c(0, 1), cbGreen)
}
draw.distr <- function(index, comment, f, r) {
  draw(paste0("distr", index), paste0("Density of the ratio distribution between ", comment), f, r, cbRed)
}

draw.ratio(1, "N(0,1) and N(0,1)", function(x) 1)
draw.distr(1, "N(0,1) and N(0,1)", function(x) dcauchy(x), c(-3, 3))

draw.ratio(2, "U(0,1) and U(0,1)", function(x) 1)
draw.distr(2, "U(0,1) and U(0,1)", function(x) ifelse(x <= 1, 0.5, 1/(2*x^2)), c(0, 10))
