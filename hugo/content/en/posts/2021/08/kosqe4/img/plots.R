...
for (name in names) {
  draw <- function(codename, estimator.mask = c(), n.mask = c()) {
    ...
    est.n <- length(unique(estimator.mask))
    #palette <- if (est.n <= length(cbPalette)) cbPalette else rainbow(est.n)
    palette <- cbPalette
    names(palette) <- estimator.mask
    for (n in ns) {
  ...
  draw("a", c("HD", "KOS-B2", "KOS-THD2"), 15)
  draw("b", c("HD", "KOS-B4", "KOS-THD4"), 15)
  draw("c", c("HD", "KOS-B6", "KOS-THD6"), 15)
  draw("d", c("HD", "KOS-THD2", "KOS-THD3", "KOS-THD4", "KOS-THD5", "KOS-THD6"), 15)
