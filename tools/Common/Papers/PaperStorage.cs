...
public class PaperStorage
{
    ...
    {
        var root = FileSystem.Papers;
        if (!root.Exists)
            root.Create();

        var mdFiles = root.EnumerateFiles("*.md");
    ...
    public void AddEntry(PublicationEntry entry)
    {
        ...
        var filePath = Path.Combine(FileSystem.Papers.FullPath, key + ".md");
    ...
    public void SaveAll()
    {
        var mdFiles = Directory.GetFiles(FileSystem.Papers.FullPath, "*.md");
        foreach (var mdFile in mdFiles)
            File.Delete(mdFile);
